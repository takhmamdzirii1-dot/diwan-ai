...
export default function App() {
  ...
  return (
    <div className="relative min-h-screen bg-[#0A0B0F] text-[#F5F6F8] selection:bg-[#1FD8B8] selection:text-[#0A0B0F]">
      {/* 1. Ambient Background Layer */}
      <AmbientMotionBackground />

      {/* 2. Navigation Header */}
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/[0.08] bg-[#0A0B0F]/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto flex h-20 items-center justify-between px-4 md:px-8">
          {/* Logo */}
          <a href="#" className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-[#1FD8B8] to-[#0EA98E] p-2 shadow-[0_0_20px_rgba(31,216,184,0.3)]">
              <Sparkles className="h-6 w-6 text-[#0A0B0F]" />
            </div>
            <div>
              <span className="text-xl font-black tracking-wider text-white">VANTRA</span>
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-medium text-white/70">
            <a href="#hero" className="transition hover:text-[#1FD8B8]">Home</a>
            <a href="#models" className="transition hover:text-[#1FD8B8]">AI Models</a>
            <a href="#pricing" className="transition hover:text-[#1FD8B8]">Pricing & DZD</a>
            <a href="#faq" className="transition hover:text-[#1FD8B8]">FAQ</a>
          </nav>
          ...
          <div className="flex items-center gap-3">
            <a
              href="#pricing"
              className="hidden sm:inline-flex items-center justify-center rounded-xl border border-white/[0.12] bg-[#0E1017]/80 px-4 py-2.5 text-xs font-semibold text-white/90 hover:bg-white/[0.06] transition"
            >
              Sign In
            </a>
            <ShimmerButton
      ...
      <main>
        ...
        <section id="models" className="py-24 px-4 md:px-8 border-t border-white/[0.06]">
          <div className="max-w-7xl mx-auto space-y-12">
            <div className="text-center max-w-3xl mx-auto space-y-4">
              <div className="inline-flex items-center gap-2 rounded-full border border-white/[0.1] bg-[#0E1017]/90 px-3.5 py-1 text-xs font-semibold text-[#1FD8B8]">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                {[
                  ...
                  <button
                    ...
                    className={`rounded-xl px-4 py-2 text-xs font-bold transition ${
                      activeFilter === tab.id
                        ? 'bg-[#1FD8B8] text-[#0A0B0F] shadow-[0_0_15px_rgba(31,216,184,0.3)]'
                        : 'border border-white/[0.08] bg-[#0E1017] text-white/70 hover:bg-white/[0.04]'
                    }`}
                  >
                    {tab.label}
                  </button>
              ...
              <input
                ...
                className="w-full sm:w-72 rounded-xl border border-white/[0.09] bg-[#0E1017] px-4 py-2.5 text-xs text-white placeholder-white/40 focus:border-[#1FD8B8] focus:outline-none"
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredModels.map((model: any) => (
                <SpotlightCard
                  key={model.id}
                  className="p-6 space-y-4 hover:border-white/[0.18] transition-all"
                >
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <h3 className="text-lg font-bold text-white">{model.name}</h3>
                        <span className="rounded-full bg-white/[0.06] px-2 py-0.5 text-[10px] font-semibold text-white/60">
                          {model.provider}
                        </span>
                      </div>
                      <p className="text-xs font-semibold text-[#1FD8B8]">{model.superpower}</p>
                    </div>
                    <span className="rounded-lg bg-[#1FD8B8]/10 px-2.5 py-1 text-xs font-mono font-bold text-[#1FD8B8] border border-[#1FD8B8]/20">
                      {model.costPts}
                    </span>
                  </div>
                  ...
                  <div className="border-t border-white/[0.06] pt-4 flex items-center justify-between text-xs">
                    <span className="text-white/45">Context: {model.contextWindow}</span>
                    <a
                      href="#pricing"
        ...
        <section id="pricing" className="py-24 px-4 md:px-8 border-t border-white/[0.06]">
          <div className="max-w-7xl mx-auto space-y-12">
            <div className="text-center max-w-2xl mx-auto space-y-4">
              <div className="inline-flex items-center gap-2 rounded-full border border-white/[0.1] bg-[#0E1017]/90 px-3.5 py-1 text-xs font-semibold text-[#F5B942]">
              ...
              {[
                ...
                <SpotlightCard
                  ...
                  className={`p-8 flex flex-col justify-between space-y-6 ${
                    tier.highlight
                      ? 'border-[#1FD8B8]/40 shadow-[0_0_40px_rgba(31,216,184,0.15)] bg-[#0E1017]'
                      : ''
                  }`}
                >
                  <div className="space-y-4">
                    {tier.highlight && (
                      <span className="inline-block rounded-full bg-[#1FD8B8] px-3 py-1 text-[11px] font-extrabold text-[#0A0B0F] uppercase tracking-wider">
                    ...
                    <div className="border-y border-white/[0.08] py-4 space-y-1">
                      <div className="text-3xl font-extrabold text-white font-mono">{tier.price}</div>
                      <div className="text-sm font-bold text-[#1FD8B8] font-mono">{tier.points}</div>
                    </div>
      ...
      {/* Footer */}
      <footer className="border-t border-white/[0.08] bg-[#0A0B0F] py-12 px-4 md:px-8 text-center text-xs text-white/40">
