...
function PrimaryCard({ href, icon, label, value, context, tone = 'neutral' }: {
  ...
  return <Link href={href} prefetch={false} className={`${cardClass} group min-h-[142px] p-4 transition-[background-color,border-color] duration-150 hover:border-[var(--studio-border-strong)] hover:bg-white/[0.035]`}>
...
function SecondaryCard({ href, icon, label, value, context }: {
  ...
  return <Link href={href} prefetch={false} className={`${cardClass} group flex min-h-[108px] items-center gap-4 p-4 transition-[background-color,border-color] duration-150 hover:border-[var(--studio-border-strong)] hover:bg-white/[0.035]`}>
    <IconFrame>{icon}</IconFrame>
...
export default function AdminOverview({ result }: { result: AdminDataResult<AdminOverviewData> }) {
  ...
  return <>
    <header className="mb-5 flex flex-wrap items-start justify-between gap-4 text-start">
      <div>
    ...
    {!result.available ? <div role="alert" className={`${cardClass} flex min-h-40 flex-col items-center justify-center p-6 text-center`}>
      ...
      {attention.length > 0 && <section aria-labelledby="attention-heading" className="mb-4 flex min-h-12 flex-wrap items-center gap-x-4 gap-y-2 rounded-xl border border-amber-300/30 bg-amber-300/[0.045] px-4 py-2.5 text-start">
      ...
      <section aria-label="Operational metrics" className="mt-3 grid gap-2.5 lg:grid-cols-3">
      ...
      <section className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,2fr)_minmax(280px,1fr)]">
        <div className={`${cardClass} min-w-0 p-4`}>
          ...
          <div className="mt-3">
            {data.recentActivity.length > 0 ? data.recentActivity.slice(0, 5).map((item) => <Link key={item.id} href={item.href} prefetch={false} className="group flex min-h-[62px] items-center gap-3 border-t border-[var(--studio-border-subtle)] py-2.5 first:border-t-0">
            ...
            </Link>) : <div className="flex min-h-[180px] items-center justify-center text-center text-[12px] text-[var(--studio-text-secondary)]">No important activity yet.</div>}
          </div>
        </div>
        ...
        <div className={`${cardClass} p-3`}>
          ...
          <div className="grid gap-2">
            ...
            ].map((action) => <Link key={action.label} href={action.href} prefetch={false} className="group flex min-h-[62px] items-center gap-3 rounded-xl border border-[var(--studio-border)] bg-white/[0.018] px-3 py-2 transition-[background-color,border-color] duration-150 hover:border-[var(--studio-border-strong)] hover:bg-white/[0.045]">
