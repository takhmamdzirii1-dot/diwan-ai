...
export default function AdminProviderControls({ provider, onSaved }: {
  ...
  return <div className="mt-2 grid gap-2.5 rounded-lg border border-[var(--studio-border-subtle)] bg-black/20 p-3 md:grid-cols-2 xl:grid-cols-[1fr_1fr_120px_160px_90px_auto] xl:items-end">
    <label className="flex h-9 items-center gap-2 rounded-lg border border-[var(--studio-border)] px-3 text-[11.5px] font-semibold text-white"><input type="checkbox" checked={enabled} disabled={!provider.configured} onChange={(event) => { setEnabled(event.target.checked); setFeedback(null); }} className="h-4 w-4 accent-white" />{t('enabledControl')}</label>
    <label className="flex h-9 items-center gap-2 rounded-lg border border-red-300/20 px-3 text-[11.5px] font-semibold text-red-100"><input type="checkbox" checked={emergencyDisabled} onChange={(event) => { setEmergencyDisabled(event.target.checked); setFeedback(null); }} className="h-4 w-4 accent-red-300" />{t('killSwitch')}</label>
    ...
    <label className="grid gap-1 text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--studio-text-muted)]">{t('spendLimit')}<input value={spendLimit} inputMode="numeric" placeholder={t('noLimit')} onChange={(event) => { setSpendLimit(event.target.value); setFeedback(null); }} className={inputClass} /></label>
    ...
    {!provider.configured && <p className="text-[11px] text-amber-100/80 md:col-span-2 xl:col-span-6">{t('credentialsMissing')}</p>}
    <p className="text-[10.5px] text-[var(--studio-text-muted)] md:col-span-2 xl:col-span-6">{t('spendHelp')}</p>
    {feedback && <p role={feedback.tone === 'error' ? 'alert' : 'status'} className={`text-[11px] md:col-span-2 xl:col-span-6 ${feedback.tone === 'error' ? 'text-red-200' : 'text-emerald-200'}`}>{feedback.text}</p>}
  </div>;
}
