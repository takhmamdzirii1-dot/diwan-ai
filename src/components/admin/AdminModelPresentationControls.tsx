...
export default function AdminModelPresentationControls({ model, onSaved }: {
  ...
  return <section className="mt-3 rounded-lg border border-[var(--studio-border-subtle)] bg-black/20 p-3 text-start">
    ...
    <details className="mt-3 rounded-lg border border-[var(--studio-border-subtle)] bg-black/15 p-2.5"><summary className="cursor-pointer text-[11px] font-semibold text-[var(--studio-text-secondary)] focus-visible:ring-2 focus-visible:ring-white/40">{t('technicalDetails')}</summary><label className={`${labelClass} mt-3 max-w-xs`}>{t('sortOrder')}<input value={sortOrder} inputMode="numeric" onChange={(event) => { setSortOrder(event.target.value); setFeedback(null); }} className={inputClass} /></label></details>
