...
export default function AdminModelCreate() {
  ...
  const create = async () => {
    ...
    try {
      const response = await fetch('/api/admin/models', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ stableId: stableId.trim(), displayName: displayName.trim(), modality }) });
      const body = await response.json().catch(() => ({}));
      if (!response.ok || !body.model) throw new Error(body.error ?? 'MODEL_CREATE_FAILED');
      setStableId(''); setDisplayName(''); setFeedback({ tone: 'success', text: t('created') });
      router.refresh();
    } catch (cause) {
  ...
  return <div className="mb-3 rounded-xl border border-[var(--studio-border)] bg-[var(--studio-surface)] p-2.5">
    <button type="button" aria-expanded={open} onClick={() => setOpen((value) => !value)} className="inline-flex h-9 items-center gap-2 rounded-lg border border-[var(--studio-border-strong)] px-3 text-[12px] font-semibold text-white hover:bg-white/[0.05] focus-visible:ring-2 focus-visible:ring-white/40"><Plus className="h-4 w-4" />{t('action')}</button>
    {open && <div className="mt-3 grid gap-2 border-t border-[var(--studio-border-subtle)] pt-3 sm:grid-cols-3 sm:items-end">
      ...
      <p className="text-[10.5px] leading-relaxed text-[var(--studio-text-muted)] sm:col-span-2">{t('help')}</p>
