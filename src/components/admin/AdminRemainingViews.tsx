...
function providerStatus(row: AdminProviderRow) {
  ...
  if (row.status === 'degraded') return pill('Degraded', 'warning');
    ...
    <table className={`${table} min-w-[680px]`}><thead><tr><th>Plan</th><th>5-hour limit</th><th>Weekly limit</th><th>Fallback</th><th>Status</th></tr></thead>
      <tbody>{failed
        ...
        : ['free', 'lite', 'pro', 'max'].map((plan) => {
          ...
          const status = !rows ? 'Loading' : !row ? 'Unconfigured' : 'Configured';
...
export function RemainingProvidersView({ result }: { result: AdminDataResult<AdminProviderRow[]> }) {
  ...
  return <>{heading('Providers', 'Manage provider connections, routes, activity and diagnostics.', <AdminProviderCreate />)}{!result.available && <NoData text="Provider data is unavailable." />}
    <div className={`${card} mb-4 p-3`}><label className="relative block"><Search className="absolute left-3 top-3 h-4 w-4 text-[var(--studio-text-muted)]" /><input aria-label="Search providers" placeholder="Search providers" value={query} onChange={(event) => { setQuery(event.target.value); setPage(0); }} className={`${control} w-full pl-9`} /></label><div className="mt-3 flex flex-wrap gap-2"><select aria-label="Status" value={status} onChange={(event) => { setStatus(event.target.value); setPage(0); }} className={control}><option value="all">All statuses</option>{['ready','degraded','disabled','misconfigured','unavailable'].map((value) => <option key={value} value={value}>{value}</option>)}</select><select aria-label="Capability" value={capability} onChange={(event) => { setCapability(event.target.value); setPage(0); }} className={control}><option value="all">All capabilities</option>{capabilities.map((value) => <option key={value} value={value}>{value}</option>)}</select><select aria-label="Activity range" value={range} onChange={(event) => { setRange(event.target.value); setPage(0); }} className={control}><option value="all">Any activity</option><option value="7d">Last 7 days</option><option value="30d">Last 30 days</option></select><button type="button" onClick={() => { setQuery(''); setStatus('all'); setCapability('all'); setRange('all'); setPage(0); }} className="px-2 text-[12px] text-[var(--studio-text-secondary)] hover:text-[var(--studio-text-primary)]">Clear filters</button></div></div>
    <p className="mb-3 text-[12px] text-[var(--studio-text-secondary)]">{filtered.length} providers · {providers.filter((row) => row.enabled && !row.emergencyDisabled).length} routing enabled · {providers.filter((row) => ['degraded', 'unavailable', 'misconfigured'].includes(row.status)).length} need attention</p>
...
function readable(value: unknown, key: string) { if (value == null) return dash; if (typeof value === 'boolean') return /enabled|active|disabled|emergency|visible|configured/i.test(key) ? value ? 'Enabled' : 'Disabled' : value ? 'Yes' : 'No'; if (typeof value === 'number') return key === 'price_dzd' ? `${value.toLocaleString('en')} DA` : value.toLocaleString('en'); if (typeof value === 'string') return value; if (Array.isArray(value)) return value.length ? value.join(', ') : 'None'; return 'Updated'; }
function changeSummary(changes: ReturnType<typeof stateChanges>, fallback: string) { if (!changes.length) return fallback; const first = changes[0]; return `${first.key.replaceAll('_',' ')}: ${readable(first.before,first.key)} → ${readable(first.after,first.key)}${changes.length > 1 ? ` · +${changes.length - 1} more` : ''}`; }
