...
function WorkspaceDrawer({ title, subtitle, badges, icon, actions, compact = false, tabs, activeTab, onTab, onClose, children }: {
  title: string; subtitle?: string; badges?: React.ReactNode; icon?: React.ReactNode; actions?: React.ReactNode; compact?: boolean; tabs: readonly string[];
  ...
  return <dialog ref={ref} onClose={onClose} onClick={(event) => { if (event.target === ref.current) ref.current?.close(); }} aria-label={title} className={`fixed inset-y-0 end-0 m-0 ms-auto h-dvh max-h-dvh w-full overflow-y-auto border-s border-[var(--studio-border)] bg-[#111112] p-0 text-white shadow-[0_0_36px_rgba(0,0,0,0.2)] backdrop:bg-black/65 ${compact ? 'max-w-[600px] lg:w-[42vw]' : 'max-w-[820px] lg:w-[54vw]'}`}>
    <div className="sticky top-0 z-20 border-b border-[var(--studio-border)] bg-[#111112]"><div className="flex items-start justify-between gap-4 px-6 pb-4 pt-6"><div className="flex min-w-0 items-start gap-3 text-start">{icon && <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-[var(--studio-border)] bg-white/[0.04] text-white">{icon}</div>}<div className="min-w-0"><h2 className="truncate text-[20px] font-semibold tracking-[-0.025em]">{title}</h2>{subtitle && <p className="mt-1 truncate text-[12px] text-[var(--studio-text-secondary)]">{subtitle}</p>}{badges && <div className="mt-2 flex flex-wrap gap-2">{badges}</div>}</div></div><div className="flex shrink-0 items-center gap-1">{actions}<button type="button" aria-label="Close details" onClick={() => ref.current?.close()} className="flex h-9 w-9 items-center justify-center rounded-lg text-[var(--studio-text-secondary)] hover:bg-white/[0.06] hover:text-white focus-visible:ring-2 focus-visible:ring-white/50"><X className="h-4 w-4" /></button></div></div>
      <div role="tablist" aria-label={`${title} details`} className="flex gap-1 overflow-x-auto px-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{tabs.map((tab) => <button key={tab} type="button" role="tab" aria-selected={activeTab === tab} onClick={() => onTab(tab)} className={`h-11 shrink-0 border-b-2 px-3 text-[12px] font-medium ${activeTab === tab ? 'border-white text-white' : 'border-transparent text-[var(--studio-text-secondary)] hover:text-white'}`}>{tab}</button>)}</div></div>
...
export function ModelsView({ result }: { result: AdminDataResult<AdminModelRow[]> }) {
  ...
  return <><PageHeader title="Models" description="Manage VANTRA models, backend mappings, pricing, plan access, and availability."><AdminModelCreate /></PageHeader>{!result.available && <Notice reason={result.reason} />}
    ...
    {filtered.length ? <div className="overflow-x-auto rounded-xl border border-[var(--studio-border)] bg-[var(--studio-surface)]"><table className="w-full min-w-[1040px] table-fixed text-start text-[11.5px]"><colgroup><col className="w-[17%]" /><col className="w-[9%]" /><col className="w-[13%]" /><col className="w-[11%]" /><col className="w-[10%]" /><col className="w-[10%]" /><col className="w-[13%]" /><col className="w-[10%]" /><col className="w-[7%]" /></colgroup><thead className="border-b border-[var(--studio-border)] bg-white/[0.025]"><tr>{['Visible Model','Category','Backend','Provider','Provider Price','VANTRA Cost','Customer Cost','Plans','Status'].map((label) => <th key={label} className="px-2.5 py-2 text-[9.5px] font-semibold uppercase tracking-[0.05em] text-[var(--studio-text-secondary)]">{label}</th>)}</tr></thead><tbody>{filtered.map((model) => { const primary = model.routes.find((item) => item.enabled && !item.fallback) ?? model.routes[0]; return <tr key={model.key} tabIndex={0} onClick={() => { setSelectedKey(model.key); setTab('Details'); }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setSelectedKey(model.key); setTab('Details'); } }} className="cursor-pointer border-b border-[var(--studio-border-subtle)] last:border-0 hover:bg-white/[0.035] focus-visible:bg-white/[0.05]"><td className="px-2.5 py-2 font-semibold text-white" title={model.displayName}><div className="flex min-w-0 items-center gap-2">{model.mediaUrl && <img src={model.mediaUrl} alt="" className="h-4 w-4 shrink-0 rounded object-contain" />}<span className="truncate">{model.displayName}</span></div></td><td className="px-2.5 py-2 text-[var(--studio-text-secondary)]">{humanizeIdentifier(model.modality)}</td><td className="truncate px-2.5 py-2" title={primary?.providerModelId ?? model.modelId}>{primary?.providerModelId ?? model.modelId}</td><td className="truncate px-2.5 py-2" title={primary?.providerId ?? model.provider}>{primary?.providerId ?? model.provider}</td><td className="px-2.5 py-2"><ProviderEconomics row={model} /></td><td className="px-2.5 py-2"><ModelOperatorCost row={model} /></td><td className="px-2.5 py-2 tabular-nums">{customerCost(model)}</td><td className="px-2.5 py-2"><div className="flex flex-wrap gap-1">{model.allowedPlans.length ? model.allowedPlans.map((plan) => <span key={plan} className="rounded-full border border-[var(--studio-border)] bg-white/[0.04] px-1.5 py-0.5 text-[9.5px] text-white">{humanizeIdentifier(plan)}</span>) : '—'}</div></td><td className="px-2.5 py-2"><Status value={model.archived ? 'archived' : model.enabled ? 'active' : 'disabled'} /></td></tr>; })}</tbody></table></div> : <Empty label="No models match these filters." />}
    {selected && <WorkspaceDrawer title={selected.displayName} subtitle={`${humanizeIdentifier(selected.modality)} · ${selected.archived ? 'Archived' : selected.enabled ? 'Active' : 'Disabled'} · ${selected.visibleInStudio ? 'Visible in Studio' : 'Hidden from Studio'}`} icon={selected.mediaUrl ? <img src={selected.mediaUrl} alt="" className="h-full w-full object-contain p-1.5" /> : selected.modality === 'video' ? <Video className="h-5 w-5" /> : selected.modality === 'image' ? <ImageIcon className="h-5 w-5" /> : <MessageCircle className="h-5 w-5" />} actions={<details className="relative"><summary aria-label="Model actions" className="flex h-9 w-9 cursor-pointer list-none items-center justify-center rounded-lg border border-[var(--studio-border)] text-[var(--studio-text-secondary)] hover:text-white [&::-webkit-details-marker]:hidden"><MoreHorizontal className="h-4 w-4" /></summary><div className="absolute end-0 top-10 z-30 w-44 rounded-lg border border-[var(--studio-border)] bg-[#202021] p-1 shadow-lg"><button type="button" onClick={() => { setTab('Details'); setEditSection('identity'); }} className="w-full rounded px-3 py-2 text-start text-[11px] hover:bg-white/[0.06]">Edit model</button><Link href={'/admin/jobs?model=' + encodeURIComponent(selected.modelId) + '&range=7d'} className="block rounded px-3 py-2 text-[11px] hover:bg-white/[0.06]">View backend jobs</Link></div></details>} tabs={['Details','Plans','Usage','Cost History']} activeTab={tab} onTab={(next) => { setTab(next); setEditSection(null); }} onClose={() => { setSelectedKey(null); setEditSection(null); }}>
      {tab === 'Details' && <div className="space-y-3">
        <ModelDetailCard title="Model Identity" onEdit={() => setEditSection(editSection === 'identity' ? null : 'identity')}>
          <ModelTile label="Visible Model">{selected.displayName}</ModelTile><ModelTile label="Category">{selected.category ?? '—'}</ModelTile><ModelTile label="Modality">{humanizeIdentifier(selected.modality)}</ModelTile><ModelTile label="Visibility">{selected.visibleInStudio ? 'Visible' : 'Hidden'}</ModelTile><ModelTile label="Status"><Status value={selected.archived ? 'archived' : selected.enabled ? 'active' : 'disabled'} /></ModelTile><ModelTile label="Availability note">{selected.availabilityLabel ?? '—'}</ModelTile>
        </ModelDetailCard>
        {editSection === 'identity' && <div className="space-y-3"><AdminModelPresentationControls model={selected} onSaved={(presentation) => setModels((current) => current.map((item) => item.key === selected.key ? { ...item, displayName: presentation.displayName, shortDescription: presentation.shortDescription, mediaUrl: presentation.mediaUrl, category: presentation.category, sortOrder: presentation.sortOrder, visibleInStudio: presentation.visibleInStudio, availabilityLabel: presentation.availabilityLabel, updatedAt: presentation.updatedAt, persisted: true } : item))} /><AdminModelCapabilitiesControls model={selected} onSaved={(capabilities, updatedAt) => setModels((current) => current.map((item) => item.key === selected.key ? { ...item, capabilities, updatedAt, persisted: true } : item))} /></div>}
        <ModelDetailCard title="Routing" onEdit={() => setEditSection(editSection === 'routing' ? null : 'routing')}>
          <ModelTile label="Backend">{route?.providerModelId ?? selected.modelId}</ModelTile><ModelTile label="Provider">{providerName ?? '—'}</ModelTile><ModelTile label="Provider Route">{route?.providerId ?? '—'}</ModelTile><ModelTile label="Primary Route">{selected.priority === 'primary' ? 'Yes' : 'No'}</ModelTile><ModelTile label="Fallback Route" wide>{selected.routes.find((item) => item.enabled && item.fallback)?.providerId ?? '—'}</ModelTile>
        </ModelDetailCard>
        {editSection === 'routing' && <div className="space-y-3"><p className="text-[11px] text-[var(--studio-text-secondary)]">Routing changes affect future requests. Recorded jobs remain historical.</p><AdminModelControls model={selected} mode="routing" onSaved={saveConfig} /><AdminModelRouteControls modelKey={selected.key} routes={selected.routes} providerOptions={selected.providerOptions} onCreated={(createdRoute) => setModels((current) => current.map((item) => item.key === selected.key ? { ...item, routes: [...item.routes, createdRoute] } : item))} onSaved={(savedRoute) => setModels((current) => current.map((item) => item.key === selected.key ? { ...item, routes: item.routes.map((itemRoute) => itemRoute.id === savedRoute.id ? { ...itemRoute, ...savedRoute } : itemRoute) } : item))} /></div>}
        ...
        {editSection === 'pricing' && <div className="space-y-3"><p className="text-[11px] text-[var(--studio-text-secondary)]">Customer cost changes affect future usage. Historical jobs and recorded costs remain unchanged.</p><AdminModelControls model={selected} mode="pricing" onSaved={saveConfig} /></div>}
        <ModelDetailCard title="Configuration" onEdit={() => setEditSection(editSection === 'configuration' ? null : 'configuration')}><ModelTile label="Last Updated"><DateValue value={selected.updatedAt} /></ModelTile><ModelTile label="Notes" wide>{selected.shortDescription ?? '—'}</ModelTile></ModelDetailCard>
        {editSection === 'configuration' && <AdminModelPresentationControls model={selected} onSaved={(presentation) => setModels((current) => current.map((item) => item.key === selected.key ? { ...item, displayName: presentation.displayName, shortDescription: presentation.shortDescription, mediaUrl: presentation.mediaUrl, category: presentation.category, sortOrder: presentation.sortOrder, visibleInStudio: presentation.visibleInStudio, availabilityLabel: presentation.availabilityLabel, updatedAt: presentation.updatedAt, persisted: true } : item))} />}
      ...
      {tab === 'Plans' && <div className="space-y-4"><ModelDetailCard title="Plan access">{['free','lite','pro','max'].map((plan) => <ModelTile key={plan} label={humanizeIdentifier(plan)}>{selected.allowedPlans.includes(plan as AdminModelRow['allowedPlans'][number]) ? 'Allowed' : 'Not allowed'}</ModelTile>)}</ModelDetailCard><AdminModelControls model={selected} mode="pricing" onSaved={saveConfig} /><p className="text-[11px] text-[var(--studio-text-muted)]">Customer cost changes affect future usage. Historical jobs and recorded costs remain unchanged.</p></div>}
      {tab === 'Usage' && <div className="rounded-xl border border-[var(--studio-border)] bg-white/[0.015] p-5"><h3 className="text-[14px] font-semibold text-white">Usage Overview</h3><p className="mt-3 max-w-xl text-[12px] leading-relaxed text-[var(--studio-text-secondary)]">Per-visible-model totals are unavailable because current job records identify the backend model. Multiple visible models can share that backend.</p><Link href={'/admin/jobs?model=' + encodeURIComponent(selected.modelId) + '&range=7d'} prefetch={false} className="mt-5 inline-flex min-h-9 items-center rounded-lg border border-[var(--studio-border)] px-3 text-[12px] font-semibold text-white hover:bg-white/[0.05]">View backend jobs in Jobs &amp; Usage</Link></div>}
      {tab === 'Cost History' && <div className="rounded-xl border border-[var(--studio-border)] bg-white/[0.015] p-4"><h3 className="text-[14px] font-semibold text-white">Recorded model changes</h3>{selected.audit.length ? selected.audit.map((item) => <div key={item.id} className="border-b border-[var(--studio-border-subtle)] py-3 text-[12px]"><p className="font-medium text-white">{humanizeIdentifier(item.action)}</p><p className="mt-1 text-[11px] text-[var(--studio-text-muted)]"><DateValue value={item.createdAt} /></p>{item.previousState?.customer_credit_price !== item.newState?.customer_credit_price && <p className="mt-1 text-[11px] text-[var(--studio-text-secondary)]">Customer cost: {String(item.previousState?.customer_credit_price ?? '—')} → {String(item.newState?.customer_credit_price ?? '—')}</p>}</div>) : <Empty label="No recorded model changes." />}</div>}
    </WorkspaceDrawer>}</>;
}
