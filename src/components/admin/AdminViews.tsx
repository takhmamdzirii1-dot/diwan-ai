...
export function ModelsView({ result }: { result: AdminDataResult<AdminModelRow[]> }) {
  ...
  const currentPage = Math.min(page, Math.max(0, Math.ceil(filtered.length / 18) - 1));
  const visibleModels = filtered.slice(currentPage * 18, (currentPage + 1) * 18);
  ...
  return <><PageHeader title="Models" description="Manage VANTRA models, backend mappings, pricing, plan access, and availability."><AdminModelCreate /></PageHeader>{!result.available && <Notice reason={result.reason} />}
    ...
    {selected && <WorkspaceDrawer segmented title={editSection ? 'Edit Model' : selected.displayName} subtitle={`${humanizeIdentifier(selected.modality)} · ${selected.archived ? 'Archived' : selected.enabled ? 'Active' : 'Disabled'} · ${selected.visibleInStudio ? 'Visible in Studio' : 'Hidden from Studio'}`} icon={selected.mediaUrl ? <img src={selected.mediaUrl} alt="" className="h-full w-full object-contain p-1.5" /> : selected.modality === 'video' ? <Video className="h-5 w-5" /> : selected.modality === 'image' ? <ImageIcon className="h-5 w-5" /> : <MessageCircle className="h-5 w-5" />} actions={<details className="relative"><summary aria-label="Model actions" className="flex h-9 w-9 cursor-pointer list-none items-center justify-center rounded-lg border border-[var(--studio-border)] text-[var(--studio-text-secondary)] hover:text-white [&::-webkit-details-marker]:hidden"><MoreHorizontal className="h-4 w-4" /></summary><div className="absolute end-0 top-10 z-30 w-44 rounded-lg border border-[var(--studio-border)] bg-[#202021] p-1 shadow-lg"><button type="button" onClick={() => { setTab('Details'); setEditSection('identity'); }} className="w-full rounded px-3 py-2 text-start text-[11px] hover:bg-white/[0.06]">Edit model</button><Link href={'/admin/jobs?model=' + encodeURIComponent(selected.modelId) + '&range=7d'} className="block rounded px-3 py-2 text-[11px] hover:bg-white/[0.06]">View backend jobs</Link></div></details>} tabs={editSection ? ['Details','Pricing','Plan Access','Advanced'] : ['Details','Plans','Usage','Cost History']} activeTab={editSection ? ({ identity: 'Details', pricing: 'Pricing', configuration: 'Plan Access', routing: 'Advanced' }[editSection]) : tab} onTab={(next) => { if (editSection) setEditSection(({ Details: 'identity', Pricing: 'pricing', 'Plan Access': 'configuration', Advanced: 'routing' } as const)[next as 'Details' | 'Pricing' | 'Plan Access' | 'Advanced']); else setTab(next); }} onClose={() => { setSelectedKey(null); setEditSection(null); }}>
      {!editSection && tab === 'Details' && <div className="space-y-3">
        ...
        <ModelDetailCard title="Configuration" onEdit={() => setEditSection('identity')}><ModelTile label="Last Updated"><DateValue value={selected.updatedAt} /></ModelTile><ModelTile label="Notes" wide>{selected.shortDescription ?? '—'}</ModelTile></ModelDetailCard>
