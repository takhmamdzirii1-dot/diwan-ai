...
import AdminFreeAccess, { type FreeAccessDetail } from './AdminFreeAccess';
...
import { modelNeedsAttention, modelStatusBadgeOf, modelStatusOf } from '@/lib/admin/model-status';
...
function Status({ value }: { value: string }) {
  ...
  const success = ['healthy', 'ready', 'completed', 'active', 'enabled', 'available', 'approved'].includes(value);
...
type ModelConfigUpdate = {
  ...
  allowedPlans: AdminModelRow['allowedPlans'];
  planAccess?: AdminModelRow['planAccess'];
  updatedAt: string;
};

function ModelQuickActions({ model, onConfigSaved, onPresentationSaved }: {
  model: AdminModelRow;
  onConfigSaved: (config: ModelConfigUpdate) => void;
  onPresentationSaved: (presentation: Pick<AdminModelRow, 'displayName' | 'shortDescription' | 'mediaUrl' | 'category' | 'sortOrder' | 'visibleInStudio' | 'availabilityLabel' | 'updatedAt'>) => void;
}) {
  const [busy, setBusy] = useState<'runtime' | 'visibility' | null>(null);
  const [feedback, setFeedback] = useState<string | null>(null);
  const updateRuntime = async () => {
    const enabled = !model.enabled;
    setBusy('runtime'); setFeedback(null);
    try {
      const response = await fetch('/api/admin/models', { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({
        modelKey: model.key, enabled, routingRole: enabled ? model.priority : 'unassigned',
        customerCreditPrice: model.creditPrice, allowedPlans: model.allowedPlans,
      }) });
      const body = await response.json().catch(() => ({}));
      if (!response.ok || !body.config) throw new Error(body.error ?? 'MODEL_CONFIG_UPDATE_FAILED');
      onConfigSaved(body.config); setFeedback(enabled ? 'Model enabled.' : 'Model disabled.');
    } catch (cause) { setFeedback(cause instanceof Error ? cause.message : 'Model status could not be saved.'); }
    finally { setBusy(null); }
  };
  const updateVisibility = async () => {
    const visibleInStudio = !model.visibleInStudio;
    setBusy('visibility'); setFeedback(null);
    try {
      const response = await fetch('/api/admin/model-presentation', { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({
        modelKey: model.key, displayName: model.displayName, shortDescription: model.shortDescription ?? '',
        mediaUrl: model.mediaUrl ?? '', category: model.category ?? '', sortOrder: model.sortOrder,
        visibleInStudio, availabilityLabel: model.availabilityLabel ?? '',
      }) });
      const body = await response.json().catch(() => ({}));
      if (!response.ok || !body.presentation) throw new Error(body.error ?? 'MODEL_PRESENTATION_UPDATE_FAILED');
      onPresentationSaved(body.presentation); setFeedback(visibleInStudio ? 'Model shown in Studio.' : 'Model hidden from Studio.');
    } catch (cause) { setFeedback(cause instanceof Error ? cause.message : 'Studio visibility could not be saved.'); }
    finally { setBusy(null); }
  };
  return <div className="flex flex-wrap items-center gap-2 rounded-xl border border-[var(--studio-border)] bg-white/[0.018] p-3">
    <button type="button" disabled={busy !== null || !model.activationSupported} onClick={updateRuntime} className="h-9 rounded-lg bg-white px-3 text-[11.5px] font-semibold text-black disabled:cursor-not-allowed disabled:opacity-45">{busy === 'runtime' ? 'Saving…' : model.enabled ? 'Disable model' : 'Enable model'}</button>
    <button type="button" disabled={busy !== null} onClick={updateVisibility} className="h-9 rounded-lg border border-[var(--studio-border)] px-3 text-[11.5px] font-semibold text-white hover:bg-white/[0.06] disabled:opacity-45">{busy === 'visibility' ? 'Saving…' : model.visibleInStudio ? 'Hide from Studio' : 'Show in Studio'}</button>
    {feedback && <span role="status" className="text-[10.5px] text-[var(--studio-text-secondary)]">{feedback}</span>}
  </div>;
}
...
export function ModelsView({ result }: { result: AdminDataResult<AdminModelRow[]> }) {
  ...
  const [overviewEditor, setOverviewEditor] = useState<'presentation' | 'pricing' | null>(null);
  ...
  const filtered = models.filter((model) => {
    ...
    if (planFilter !== 'all' && model.planAccess[planFilter as ModelPlanCode].state === 'locked') return false;
  ...
  const saveConfig = (config: ModelConfigUpdate) => setModels((current) => current.map((item) => item.key === config.modelKey ? { ...item, enabled: config.enabled, priority: config.routingRole, creditPrice: config.customerCreditPrice, allowedPlans: config.allowedPlans, planAccess: config.planAccess ?? item.planAccess, updatedAt: config.updatedAt, persisted: true } : item));
  const savePresentation = (presentation: Pick<AdminModelRow, 'displayName' | 'shortDescription' | 'mediaUrl' | 'category' | 'sortOrder' | 'visibleInStudio' | 'availabilityLabel' | 'updatedAt'>) => setModels((current) => current.map((item) => item.key === selected?.key ? { ...item, ...presentation, persisted: true } : item));
  ...
  return <><PageHeader title="Models" description="Manage VANTRA models, backend mappings, pricing, plan access, and availability."><AdminModelCreate /></PageHeader>{!result.available && <Notice reason={result.reason} />}
    ...
    {selected && <WorkspaceDrawer segmented title={selected.displayName} subtitle={`${humanizeIdentifier(selected.modality)} · ${selected.catalogOnly ? 'Unconfigured' : selected.archived ? 'Archived' : selected.enabled ? 'Active' : 'Disabled'} · ${selected.catalogOnly ? 'Catalog only' : selected.visibleInStudio ? 'Visible in Studio' : 'Hidden from Studio'}`} icon={<ModelBrandIcon url={selected.mediaUrl} name={brandOf(selected)} size={24} />} tabs={selected.catalogOnly ? ['Overview'] : ['Overview','Access','Capabilities','Routes','Activity']} activeTab={tab === 'Details' ? 'Overview' : tab} onTab={(next) => { setTab(next); setOverviewEditor(null); }} onClose={() => { setSelectedKey(null); setOverviewEditor(null); }}>
      ...
      {!selected.catalogOnly && (tab === 'Overview' || tab === 'Details') && <div className="space-y-3">
        <ModelQuickActions model={selected} onConfigSaved={saveConfig} onPresentationSaved={savePresentation} />
        <ModelDetailCard title="Presentation" onEdit={() => setOverviewEditor(overviewEditor === 'presentation' ? null : 'presentation')}>
          <ModelTile label="Display name">{selected.displayName}</ModelTile><ModelTile label="Brand">{brandOf(selected)}{selected.category?.trim() ? '' : ' (inferred)'}</ModelTile><ModelTile label="Modality">{humanizeIdentifier(selected.modality)}</ModelTile><ModelTile label="Visible in Studio">{selected.visibleInStudio ? 'Visible' : 'Hidden'}</ModelTile><ModelTile label="Availability note" wide>{selected.availabilityLabel ?? '—'}</ModelTile>
        </ModelDetailCard>
        {overviewEditor === 'presentation' && <AdminModelPresentationControls model={selected} onSaved={(presentation) => { savePresentation(presentation); setOverviewEditor(null); }} />}
        <ModelDetailCard title="Runtime & customer pricing" onEdit={() => setOverviewEditor(overviewEditor === 'pricing' ? null : 'pricing')}>
          <ModelTile label="Runtime status"><Status value={modelStatusBadgeOf(selected)} /></ModelTile><ModelTile label="Customer price">{customerCost(selected)}</ModelTile><ModelTile label="Provider cost"><ProviderEconomics row={selected} /></ModelTile><ModelTile label="VANTRA cost"><ModelOperatorCost row={selected} /></ModelTile>
        </ModelDetailCard>
        {overviewEditor === 'pricing' && <AdminModelControls model={selected} mode="pricing" onSaved={(config) => { saveConfig(config); setOverviewEditor(null); }} />}
        <ModelDetailCard title="Primary route" onEdit={() => { setTab('Routes'); setOverviewEditor(null); }}>
          <ModelTile label="Provider">{providerName ?? '—'}</ModelTile><ModelTile label="Backend model ID">{route?.providerModelId ?? selected.modelId}</ModelTile><ModelTile label="Routing role">{humanizeIdentifier(selected.priority)}</ModelTile><ModelTile label="Route status">{route ? route.enabled && route.configured && route.providerEnabled ? 'Ready' : route.enabled ? 'Unavailable' : 'Disabled' : 'No route'}</ModelTile>
        </ModelDetailCard>
        <TechnicalDetails><TechnicalId label="Model key" value={selected.key} /><TechnicalId label="Internal model ID" value={selected.modelId} /><TechnicalId label="Route ID" value={route?.id ?? '—'} /><p className="mt-1">Last updated: <DateValue value={selected.updatedAt} /></p></TechnicalDetails>
        <AdminModelLifecycleControls modelKey={selected.key} archived={selected.archived} />
      </div>}
      {!selected.catalogOnly && tab === 'Access' && <div className="space-y-3"><p className="text-[12px] text-[var(--studio-text-secondary)]">Configure Included, Trial, or Locked access for each plan. Trial access requires an allowance; MAX remains frozen.</p><AdminModelControls model={selected} mode="plans" onSaved={saveConfig} /></div>}
      {!selected.catalogOnly && tab === 'Capabilities' && <AdminModelCapabilitiesControls model={selected} onSaved={(patch) => setModels((current) => current.map((item) => item.key === selected.key ? { ...item, ...patch, persisted: true } : item))} />}
      {!selected.catalogOnly && tab === 'Routes' && <div className="space-y-3"><p className="text-[12px] text-[var(--studio-text-secondary)]">Routing changes affect future requests. Recorded jobs remain historical.</p><AdminModelControls model={selected} mode="routing" onSaved={saveConfig} /><AdminModelRouteControls modelKey={selected.key} routes={selected.routes} providerOptions={selected.providerOptions} onCreated={(createdRoute) => setModels((current) => current.map((item) => item.key === selected.key ? { ...item, routes: [...item.routes, createdRoute] } : item))} onSaved={(savedRoute) => setModels((current) => current.map((item) => item.key === selected.key ? { ...item, routes: item.routes.map((itemRoute) => itemRoute.id === savedRoute.id ? { ...itemRoute, ...savedRoute } : itemRoute) } : item))} /></div>}
      {!selected.catalogOnly && tab === 'Activity' && <div className="space-y-3">
        <section className="rounded-xl border border-[var(--studio-border)] bg-[var(--studio-card)] p-4">
          <div className="flex items-center justify-between gap-3"><h3 className="text-[15px] font-semibold">Usage &amp; cost</h3><Link href={'/admin/jobs?model=' + encodeURIComponent(selected.modelId) + '&range=7d'} prefetch={false} className="rounded-lg border border-[var(--studio-border)] px-3 py-2 text-[11px] font-semibold hover:bg-[var(--studio-hover)]">View backend jobs</Link></div>
          <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4"><ModelTile label="Jobs">—</ModelTile><ModelTile label="Success / failure">—</ModelTile><ModelTile label="VANTRA cost"><ModelOperatorCost row={selected} /></ModelTile><ModelTile label="Provider cost"><ProviderEconomics row={selected} /></ModelTile></div>
          <p className="mt-3 text-[11px] leading-relaxed text-[var(--studio-text-secondary)]">Multiple visible models can share one backend identifier, so model-specific job totals remain unavailable until stable model keys are present in job attribution.</p>
        </section>
        <section className="rounded-xl border border-[var(--studio-border)] bg-white/[0.015] p-4"><h3 className="text-[14px] font-semibold text-white">Recent changes &amp; cost history</h3>{selected.audit.length ? selected.audit.map((item) => <div key={item.id} className="border-b border-[var(--studio-border-subtle)] py-3 text-[12px]"><p className="font-medium text-white">{humanizeIdentifier(item.action)}</p><p className="mt-1 text-[11px] text-[var(--studio-text-muted)]"><DateValue value={item.createdAt} /></p>{item.previousState?.customer_credit_price !== item.newState?.customer_credit_price && <p className="mt-1 text-[11px] text-[var(--studio-text-secondary)]">Customer cost: {String(item.previousState?.customer_credit_price ?? '—')} → {String(item.newState?.customer_credit_price ?? '—')}</p>}</div>) : <Empty label="No recorded model changes." />}</section>
      </div>}
    </WorkspaceDrawer>}</>;
}
...
type UserDetailData = {
  freeAccess: FreeAccessDetail | null;
  balances: {
    balance: string;
...
function AdminUserDetail({ user, onClose, onStatusChanged, onCreditChanged }: {
  ...
  return <dialog ref={drawerRef} onClose={onClose} onCancel={(event) => { if (!canLeave()) event.preventDefault(); }} onClick={(event) => { if (event.target === drawerRef.current) closeDrawer(); }} aria-label={`User details for ${user.email}`} className="fixed inset-y-0 end-0 m-0 ms-auto h-dvh max-h-dvh w-full max-w-[960px] overflow-y-auto border-s border-[var(--studio-border)] bg-[#111112] p-0 text-white shadow-[0_0_36px_rgba(0,0,0,0.2)] backdrop:bg-black/65 lg:w-[58vw]">
    ...
    <div className="px-6 pb-8">
      ...
      {tab === 'overview' && <div>
        ...
        <UserOverviewSection icon={<ImageIcon className="h-5 w-5" />} title="Media" description="Current stored allowances and balances.">
          {plan === 'free' ? <>
            ...
            <p className="pt-3 text-[11px] text-[var(--studio-text-muted)]">Free Media Allowance: 5 images and 1 video, one time, with no expiry.</p>
          </> : plan === 'lite' ? <>
      ...
      {tab === 'security' && <div className="space-y-4 py-6"><p className="text-[12px] text-[var(--studio-text-secondary)]">{t('users.securityHelp')}</p><AdminFreeAccess userId={user.id} detail={data?.freeAccess} onChanged={(next) => setData((current) => current ? { ...current, freeAccess: { ...current.freeAccess, ...next } } : current)} /><AdminUserActions userId={user.id} status={user.status} isOwner={user.isOwner} onChanged={onStatusChanged} /></div>}
