...
export default function MediaLibrary() {
  ...
  const visible = useMemo(() => {
    ...
    return items.filter((item) => filter === 'all' || item.kind === filter.slice(0, -1)).filter((item) => !normalized || `${item.prompt ?? ''} ${item.model ?? ''}`.toLocaleLowerCase().includes(normalized)).sort((a, b) => sort === 'newest' ? b.createdAt - a.createdAt : a.createdAt - b.createdAt);
