...
export default function ArtifactSpreadsheetPreview({ file: sourceFile, initialArtifact, locale, onClose, onAnalyze, inline = false }: { file?: File; initialArtifact?: SpreadsheetArtifact; locale: string; onClose: () => void; onAnalyze: (prompt: string) => void | Promise<void>; inline?: boolean }) {
  ...
  useEffect(() => { if (!sourceFile) return; let cancelled = false; void import('@/lib/artifacts/spreadsheet-io').then(({ importSpreadsheet }) => importSpreadsheet(sourceFile, locale)).then((result) => { if (!cancelled) setArtifact(result); }).catch((cause) => { if (!cancelled) setError(cause instanceof Error ? cause.message : 'Could not open workbook.'); }); return () => { cancelled = true; }; }, [sourceFile, locale]);
  ...
  if (inline) return <section aria-label={artifact?.title ?? t.sheet} className="w-full rounded-xl border border-[var(--studio-border)] bg-[var(--studio-surface)] p-3 text-white" dir={artifact?.direction ?? 'auto'}>
    <header className="mb-3 flex items-center justify-between gap-3"><h3 className="truncate text-sm font-semibold">{artifact?.title}</h3><button type="button" disabled={!artifact} onClick={() => void exportXlsx()} className="rounded-lg border border-white/20 px-3 py-2 text-xs disabled:opacity-50">{t.xlsx}</button></header>
