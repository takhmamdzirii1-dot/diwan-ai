...
export default function ArtifactSpreadsheetPreview({ file, locale, onClose, onAnalyze }: { file: File; locale: string; onClose: () => void; onAnalyze: (prompt: string) => void | Promise<void> }) {
  const t = copy[locale as keyof typeof copy] ?? copy.en;
  const chartLocaleIndex = locale === 'ar' ? 2 : locale === 'fr' ? 1 : 0;
  const [artifact, setArtifact] = useState<SpreadsheetArtifact | null>(null);
  ...
  useEffect(() => { let cancelled = false; void import('@/lib/artifacts/spreadsheet-io').then(({ importSpreadsheet }) => importSpreadsheet(file, locale)).then((result) => { if (!cancelled) setArtifact(result); }).catch((cause) => { if (!cancelled) setError(cause instanceof Error ? cause.message : 'Could not open workbook.'); }); return () => { cancelled = true; }; }, [file, locale]);
  const sheet = artifact?.sheets[sheetIndex];
  const makeChart = () => { if (!artifact || !sheet) return; try { setExportPng(null); setChart(chartFromSheet(artifact, sheet, chartType, Math.max(0, rangeStart - 1), Math.min(sheet.rows.length, rangeEnd))); setError(''); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Could not create chart.'); } };
  const exportXlsx = async () => { if (!artifact) return; try { const { exportSpreadsheet } = await import('@/lib/artifacts/spreadsheet-io'); download(await exportSpreadsheet(artifact), `${artifact.title}.xlsx`); } catch { setError('XLSX export failed.'); } };
  const analyze = (presentationRequest = false) => { if (!artifact || !sheet) return; const context = spreadsheetContext(artifact, sheet, page * PAGE_SIZE, Math.min(sheet.rows.length, (page + 1) * PAGE_SIZE)); onClose(); void onAnalyze(presentationRequest ? `Create a concise presentation from this spreadsheet. Return ONE JSON object inside a json code fence using this exact schema: {"schemaVersion":1,"id":"presentation","type":"presentation","title":"...","language":"${locale}","direction":"${artifact.direction}","metadata":{},"slides":[{"id":"slide-1","layout":"title","title":"...","subtitle":"...","blocks":[{"kind":"text","text":"..."}]}]}. Use only text, bullets, and table blocks; at most 8 slides. Do not request more data.\n\n${context}` : `Analyze this spreadsheet and summarize the main findings. Use only the selected bounded data below; state limitations.\n\n${context}`); };
