  ...
  return (
    <CreationWorkspace
      ...
      controls={<>
          ...
          <form className="studio-creation-form space-y-5" onSubmit={submitDraft} noValidate>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              {capabilities && capabilities.aspectRatios.length > 0 && <div className="space-y-2 sm:col-span-2"><FieldLabel>{t('aspectRatio')}</FieldLabel><div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${capabilities.aspectRatios.length <= 5 ? capabilities.aspectRatios.length : Math.ceil(capabilities.aspectRatios.length / 2)}, minmax(0, 1fr))` }}>{capabilities.aspectRatios.map((ratio) => <button key={ratio} type="button" aria-pressed={aspectRatio === ratio} onClick={() => setAspectRatio(ratio)} className={cn('min-h-9 min-w-0 rounded-lg border px-1.5 text-[12px] font-semibold transition-colors duration-150 motion-reduce:transition-none', aspectRatio === ratio ? 'border-[var(--studio-accent)] bg-[var(--studio-accent)] text-[var(--studio-accent-contrast)]' : 'border-[var(--studio-border)] text-[var(--studio-text-secondary)] hover:text-[var(--studio-text-primary)]')}>{ratio}</button>)}</div></div>}
            </div>
      ...
      preview={<div className="flex min-h-[320px] w-full flex-col gap-3 lg:h-full lg:min-h-0">
        ...
        <MediaResultRail items={railItems} selectedId={result?.libraryAssetId ?? ''} kind="image" onSelect={selectRailItem} />
      </div>}
    />
  );
}
