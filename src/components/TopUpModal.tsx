...
export default function TopUpModal({ isOpen, onClose, plan }: TopUpModalProps) {
  ...
  const methodCard = (id: PaymentMethod, Icon: React.ElementType, detail: string) => {
    ...
    return <button key={id} type="button" role="radio" aria-checked={active} disabled={!available[id] || busy} onClick={() => void changeMethod(id)}
      className={'flex min-h-20 w-full items-center gap-4 rounded-2xl border-2 p-4 text-start focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--studio-accent,#fff)] ' + (active ? 'border-[var(--studio-accent,#fff)] bg-[var(--studio-hover,#29292d)]' : 'border-[var(--studio-border,#444)] hover:bg-[var(--studio-hover,#29292d)]') + (!available[id] ? ' opacity-55' : '')}>
      <Icon className="h-6 w-6 shrink-0" aria-hidden="true" /><span className="min-w-0 flex-1"><span className="block text-base font-semibold">{t(id)}</span><span className="mt-1 block text-base text-[var(--studio-text-secondary,#bbb)]">{available[id] ? detail : t('comingSoon')}</span></span>
  ...
  return <AnimatePresence>{isOpen && <div id="vantra-topup-modal-root" role="dialog" aria-modal="true" aria-labelledby="topup-title" className="fixed inset-0 z-[99999] flex items-center justify-center overflow-y-auto p-2 text-[var(--studio-text-primary,#fff)] sm:p-5">
    ...
    <motion.div ref={dialog} initial={{ opacity: 0, scale: 0.98, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98, y: 8 }} transition={{ duration: 0.2 }} className="relative z-10 my-auto flex max-h-[calc(100svh-16px)] w-full max-w-[580px] flex-col overflow-hidden rounded-3xl border border-[var(--studio-border,#444)] bg-[var(--studio-card,#151517)] shadow-[var(--studio-shadow,0_24px_80px_rgba(0,0,0,.7))] sm:max-h-[calc(100svh-40px)]">
      <header className="border-b border-[var(--studio-border,#444)] px-5 pb-4 pt-5 sm:px-7">
        <div className="flex items-start gap-3"><button type="button" onClick={back} disabled={busy || submitted || confirmation || step === 1 || (step === 2 && preselected)} aria-label={t('back')} className="flex min-h-12 min-w-12 items-center justify-center rounded-xl border border-[var(--studio-border,#444)] disabled:invisible focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--studio-accent,#fff)]"><ArrowLeft className="h-5 w-5 rtl:rotate-180" /></button><div className="min-w-0 flex-1"><h2 ref={heading} tabIndex={-1} id="topup-title" className="text-xl font-semibold outline-none">{t('title')}</h2><p className="mt-1 text-base text-[var(--studio-text-secondary,#bbb)]">{submitted ? t('pendingTitle') : step === 1 ? t('selectPlan') : step === 2 ? t('paymentMethod') : t('paymentDetails')}</p></div><button type="button" onClick={onClose} disabled={busy} aria-label={t('close')} className="flex min-h-12 min-w-12 items-center justify-center rounded-xl border border-[var(--studio-border,#444)] disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--studio-accent,#fff)]"><X className="h-5 w-5" /></button></div>
        <div className="mt-4 flex items-center justify-between text-base font-medium text-[var(--studio-text-secondary,#bbb)]"><span>{t('stepOf', { step, total: 3 })}</span><span>{Math.round(step / 3 * 100)}%</span></div><div role="progressbar" aria-valuenow={step} aria-valuemin={1} aria-valuemax={3} aria-label={t('progressLabel')} className="mt-2 h-1.5 overflow-hidden rounded-full bg-[var(--studio-border,#444)]"><div className="h-full rounded-full bg-[var(--studio-accent,#fff)] transition-[width] duration-200" style={{ width: String(step / 3 * 100) + '%' }} /></div>
      </header>
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5 text-base sm:px-7">
        {submitted ? <div role="status" className={surface + ' p-6 text-center'}><Clock3 className="mx-auto h-9 w-9" /><h3 className="mt-3 text-xl font-semibold">{t('pendingTitle')}</h3><p className="mt-3 leading-relaxed text-[var(--studio-text-secondary,#bbb)]">{t('pendingDescription')}</p>{order && <p dir="ltr" className="mt-4 rounded-xl border border-[var(--studio-border,#444)] p-3 font-semibold">{order.payment_reference}</p>}</div> : <>
          ...
          {step > 1 && selectedPlan && <section className={surface + ' p-4'} aria-label={t('selectedPlan')}><div className="flex items-start justify-between gap-3"><div><p className="text-base text-[var(--studio-text-secondary,#bbb)]">{t('selectedPlan')}</p><h3 className="mt-1 text-lg font-semibold">{selectedPlan.name}</h3></div><button type="button" onClick={() => void changePlan()} disabled={busy || confirmation || submitted} className="min-h-12 rounded-lg px-2 text-base font-semibold underline underline-offset-4 disabled:opacity-40">{t('change')}</button></div><div className="mt-3 flex flex-wrap justify-between gap-2 border-t border-[var(--studio-border-subtle,#333)] pt-3"><span>{selectedPlan.unifiedCredits.toLocaleString(locale)} {t('creditsShort')}</span><strong dir="ltr">{selectedPlan.priceDzd.toLocaleString(locale)} DA</strong></div></section>}
          {step === 2 && <div className="space-y-5"><section><div className="mb-3 flex items-center gap-2"><h3 className="text-lg font-semibold">{t('instant')}</h3>{instantAvailable && <span className="rounded-full border border-[var(--studio-border,#444)] px-2 py-1 text-base font-semibold">{t('recommended')}</span>}</div><p className="mb-3 text-base text-[var(--studio-text-secondary,#bbb)]">{instantAvailable ? t('activatesImmediately') : t('gatewayUnavailableTitle')}</p><div role="radiogroup" aria-label={t('instant')} className="space-y-3">{methodCard('edahabia', CreditCard, t('activatesImmediately'))}{methodCard('cib', CreditCard, t('activatesImmediately'))}</div></section><section><h3 className="mb-1 text-lg font-semibold">{t('manualTransfer')}</h3><p className="mb-3 text-base text-[var(--studio-text-secondary,#bbb)]">{t('reviewedWithinHours')}</p><div role="radiogroup" aria-label={t('manualTransfer')} className="space-y-3">{methodCard('baridimob', Banknote, t('reviewedWithinHours'))}{methodCard('ccp', Building2, t('reviewedWithinHours'))}</div></section></div>}
