...
/** Bundled Lobe icons are monochrome currentColor SVGs; use their silhouette with the active theme text token. */
export function ModelBrandIcon({ url, name, size = 24 }: { url?: string | null; name: string; size?: number }) {
  ...
  if (!url.startsWith('/brand/models/')) return <img src={url} alt="" style={{ width: size, height: size }} className="shrink-0 rounded object-contain" />;
