-- Finalize manual subscription periods without changing MAX commercial behavior
-- or the existing reserve -> settle -> release generation ledger.
...
create table public.subscription_periods (
  ...
  entitlement_id uuid not null unique references public.user_entitlements(id) on delete restrict,
  plan_code text not null check (plan_code in ('lite', 'pro')),
  period_starts_at timestamptz not null,
  period_ends_at timestamptz not null,
  base_allowance bigint not null check (base_allowance >= 0),
  rollover_amount bigint not null default 0 check (rollover_amount between 0 and 600),
  included_videos smallint not null default 0 check (included_videos between 0 and 4),
  eligible_unused_at_close bigint not null default 0 check (eligible_unused_at_close >= 0),
  state text not null check (state in ('scheduled', 'active', 'expired', 'replaced')),
  activated_at timestamptz,
  closed_at timestamptz,
  created_at timestamptz not null default now(),
  constraint subscription_period_positive_range check (period_ends_at > period_starts_at),
  constraint subscription_period_shape check (
    (state = 'scheduled' and activated_at is null and closed_at is null)
    or (state = 'active' and activated_at is not null and closed_at is null)
    or (state in ('expired', 'replaced') and closed_at is not null)
  )
);

create unique index subscription_periods_one_active_per_user
  on public.subscription_periods (user_id) where state = 'active';
create index subscription_periods_due_idx
  on public.subscription_periods (user_id, period_starts_at)
  where state = 'scheduled';
create index subscription_periods_history_idx
  on public.subscription_periods (user_id, period_ends_at desc);

alter table public.subscription_periods enable row level security;
revoke all on table public.subscription_periods from public, anon, authenticated;
grant select on table public.subscription_periods to service_role;

-- Bring the currently tracked Lite/Pro period under lifecycle control without
-- changing any live balance, entitlement, or historical order snapshot.
...
select
  account.user_id,
  entitlement.source_payment_order_id,
  entitlement.id,
  account.subscription_plan_code,
  greatest(entitlement.starts_at, account.subscription_period_ends_at - interval '30 days'),
  account.subscription_period_ends_at,
  greatest(account.subscription_balance - account.subscription_rollover_balance, 0),
  least(account.subscription_rollover_balance, 600),
  case when account.subscription_plan_code = 'lite' then 4 else 0 end,
  'active',
  entitlement.starts_at
from public.credits account
join public.user_entitlements entitlement
  on entitlement.id = account.subscription_entitlement_id
where account.subscription_plan_code in ('lite', 'pro')
  and account.subscription_period_ends_at is not null
on conflict (payment_order_id) do nothing;

-- Lite/Pro same-plan renewals become future entitlement periods. Upgrades and
-- expired renewals start immediately. MAX retains its existing extension shape.
...
-- Called only by trusted financial functions (and the authenticated read RPC
-- below). The credit row lock serializes expiry and boundary activation.
...
begin
  ...
  loop
    ...
    update public.credits set
      ...
      lite_video_remaining = case when v_period.plan_code = 'lite' then 4 else 0 end,
...
-- Free accounts cannot create ordinary top-ups. Historical eligibility for
-- restricted subscription offers remains unchanged.
...
begin
  ...
  if (v_plan_code = 'lite' and v_allowance <> 850)
    or (v_plan_code = 'pro' and v_allowance <> 3000)
  then
    raise exception using errcode = 'P0001', message = 'PAYMENT_APPROVAL_INTEGRITY_ERROR';
  end if;
  ...
  if v_order.order_kind = 'credit_pack' then
    ...
    if v_current_plan = v_plan_code and v_plan_code in ('lite', 'pro') then
      ...
      insert into public.subscription_periods (
        ...
        case when v_plan_code = 'lite' then 4 else 0 end,
        'scheduled'
...
-- Keep the original reservation implementation byte-for-byte in behavior and
-- place lifecycle refresh plus purchased-credit eligibility in a thin wrapper.
alter function public.reserve_credits(
  uuid, text, text, text, text, bigint, text, jsonb, jsonb, timestamptz
) rename to reserve_credits_v2_base;
...
begin
  ...
  select * into v_account from public.credits where user_id = p_user_id;

  if p_modality = 'chat' and p_amount > v_account.subscription_balance then
    select exists (
      select 1 from public.user_entitlements entitlement
      join public.payment_plans plan on plan.id = entitlement.plan_id
    ...
    if not v_has_paid_access then
      raise exception using errcode = '42501', message = 'ACTIVE_PAID_PLAN_REQUIRED_FOR_PURCHASED_CREDITS';
    end if;
  end if;

  return public.reserve_credits_v2_base(
    p_user_id, p_operation_key, p_payload_hash, p_modality, p_model_id,
    p_amount, p_pricing_version, p_pricing_snapshot, p_route_snapshot, p_expires_at
  );
end;
$$;

-- Balance reads activate a due period before exposing its allowance.
create or replace function public.get_current_credit_account()
returns table (
  balance bigint,
  subscription_balance bigint,
  purchased_balance bigint,
  free_image_remaining smallint,
  free_video_remaining smallint,
  lite_video_remaining smallint
)
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  return query
    select account.balance, account.subscription_balance, account.purchased_balance,
      account.free_image_remaining, account.free_video_remaining, account.lite_video_remaining
    from public.credits account where account.user_id = v_user_id;
end;
$$;
...
revoke all on function public.get_current_credit_account() from public, anon;
grant execute on function public.get_current_credit_account() to authenticated;

comment on table public.subscription_periods is
  'Atomic Lite/Pro paid-period schedule. Each approved payment owns exactly one 30-day period.';
comment on function public.refresh_subscription_lifecycle(uuid) is
  'Closes expired allowances and activates due renewal periods exactly once under the credit-row lock.';
comment on function public.get_current_credit_account() is
  'Authenticated balance read that first applies any due subscription-period boundary.';
